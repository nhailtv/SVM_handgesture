...
output_csv = 'Data/hand_gesture_training_data_checked.csv'
...
def process_hand_gesture_images(max_images_per_class=None, use_gpu=False):
    ...
    with open(output_csv, 'w', newline='') as out_csv:
        ...
        with tqdm(total=total_images, desc="🖼️ Xử lý ảnh", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}] {desc}') as pbar:
            total_processed = 0
            total_successful = 0
                ...
                for filename in image_files:
                    ...
                    try:
                        ...
                        if hands1 and hands2:
                            ...
                            # So sánh kết quả hai lần
                            if are_landmarks_close(lm_list1, lm_list2, threshold=15):
                                # Nếu giống nhau (ổn định), mới lưu
                                for lm in lm_list1:
                                ...
                                # Nếu khác biệt lớn, bỏ qua ảnh này
                                pass
                        images_processed += 1
                        total_processed += 1
                    except Exception as e:
